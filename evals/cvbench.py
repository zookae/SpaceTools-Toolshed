...
"""
CVBench dataset evaluator.

This module provides evaluation capabilities for the CVBench dataset,
which includes 2D Relation and 3D Depth spatial reasoning tasks.
"""
...
# Import scoring functions from VeRL
from verl.utils.reward_score.cvbench_2d_relation import compute_score as compute_score_relation
from verl.utils.reward_score.cvbench_3d_depth import compute_score as compute_score_depth
...
# Default system prompt for CVBench tasks with tools
...
class CVBenchEvaluator(BaseEvaluator):
    """Evaluates models on the CVBench dataset (2D Relation and 3D Depth tasks)."""
    ...
    def parse_example(self, row: Dict) -> Dict:
        """
        Parse a CVBench dataset row into standardized format.
        
        Extracts question, ground truth, image, and question type from the CVBench schema.
        ...
        """
        # Extract CVBench-specific fields
        question = row['extra_info']['question']
        ground_truth = row['reward_model']['ground_truth']
        data_source = row['data_source']
        
        # Determine question type from data_source
        if "2D_Relation" in data_source:
            question_type = "cvbench_rel"
        elif "3D_Depth" in data_source:
            question_type = "cvbench_depth"
        else:
            raise ValueError(f"Unknown CVBench data_source: {data_source}")
    ...
    def score_prediction(self, prediction: str, parsed_example: Dict, messages: Optional[List[Dict]] = None) -> Dict[str, Any]:
        """
        Score a model prediction using appropriate CVBench scoring function.
        ...
        try:
            ...
            # Use appropriate scoring function based on question type
            if question_type == "cvbench_rel":
                score_result = compute_score_relation(
                    solution_str=prediction,
                    ground_truth=parsed_example['ground_truth']
                )
            elif question_type == "cvbench_depth":
                score_result = compute_score_depth(
                    solution_str=prediction,
                    ground_truth=parsed_example['ground_truth']
                )
            else:
    ...
    def get_result_metadata(self, parsed_example: Dict, score: float) -> Dict:
        """
        Extract CVBench-specific metadata for results.
        ...
        Returns:
            Dict with ground_truth, original question, question_type, and original system prompt
        """
        return {
            'ground_truth': parsed_example['ground_truth'],
    ...
    def compute_custom_metrics(self, results: List[Dict]) -> Dict[str, Any]:
        """
        Compute custom metrics for CVBench, including per-question-type breakdown.
        
        Args:
            results: List of valid result dictionaries
            
        Returns:
            Dict with per-question-type metrics
        """
        ...
        for qtype, stats in type_stats.items():
            if stats['total'] > 0:
                type_accuracies[qtype] = stats['correct'] / stats['total']
            else:
                type_accuracies[qtype] = 0.0
        
        return {
            'per_question_type': type_accuracies,
            'question_type_counts': {qtype: stats['total'] for qtype, stats in type_stats.items()}
        }
    
    def should_include_in_training_data(self, result: Dict[str, Any]) -> bool:
        """
        Determine whether a CVBench result should be included in training data.
    ...
    def print_results_summary(self, results: Dict[str, Any]):
        """Print a formatted summary of CVBench evaluation results."""
        print("\n" + "="*60)
        ...
        if 'per_question_type' in results:
            print(f"\nPer Question Type:")
            for qtype, acc in results['per_question_type'].items():
                count = results.get('question_type_counts', {}).get(qtype, 0)
                print(f"  {qtype}: {acc:.2%} ({count} examples)")
